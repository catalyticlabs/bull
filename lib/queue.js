...
Queue.prototype.add = function(name, data, opts) {
  ...
  this.emit('log', `Adding job ${name} with options`, { opts });
...
Queue.prototype.processJob = function(job, notFetch = false) {
  ...
  const handleFailed = err => {
    ...
    this.emit('log', 'Job failed', { job, error });
    ...
    return job
      ...
      .catch(err => {
        this.emit('log', 'Error marking job as failed', { job, err, error });
        throw err;
      });
  };
  ...
  if (!handler) {
    ...
    this.emit('log', `Process job with timeout ${timeoutMs}`, { job });
    if (timeoutMs) {
